...
pub const USER_DATA_DIR_NAME: &str = "SlopZed";
...
pub const USER_DATA_DIR_NAME_LOWERCASE: &str = {
    assert!(
        !USER_DATA_DIR_NAME.is_empty(),
        "USER_DATA_DIR_NAME must not be empty"
    );
    assert!(
        USER_DATA_DIR_NAME.as_bytes().is_ascii(),
        "USER_DATA_DIR_NAME must be ASCII"
    );
    const BYTES: [u8; USER_DATA_DIR_NAME.len()] = {
        let mut bytes = [0u8; USER_DATA_DIR_NAME.len()];
        let mut i = 0;
        while i < USER_DATA_DIR_NAME.len() {
            assert!(
                USER_DATA_DIR_NAME.as_bytes()[i] != b'/'
                    && USER_DATA_DIR_NAME.as_bytes()[i] != b'\\',
                "USER_DATA_DIR_NAME must not contain path separators",
            );
            assert!(
                USER_DATA_DIR_NAME.as_bytes()[i] >= 0x20,
                "USER_DATA_DIR_NAME must not contain control characters"
            );
            bytes[i] = USER_DATA_DIR_NAME.as_bytes()[i];
            i += 1;
        }
        bytes.make_ascii_lowercase();
        bytes
...
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            .join(USER_DATA_DIR_NAME_LOWERCASE)
        } else {
            home_dir()
                .join(".config")
                .join(USER_DATA_DIR_NAME_LOWERCASE)
...
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join(USER_DATA_DIR_NAME_LOWERCASE)
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs").join(USER_DATA_DIR_NAME)
        } else {
            data_dir().join("logs")
        }
    })
...
pub fn log_file() -> &'static PathBuf {
    ...
    LOG_FILE.get_or_init(|| logs_dir().join(format!("{}.log", USER_DATA_DIR_NAME)))
}

/// Returns the path to the `Zed.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join(format!("{}.log.old", USER_DATA_DIR_NAME)))
}
...
pub const GLOBAL_AGENTS_FILE_DISPLAY: &str =
    const_format::concatcp!("%APPDATA%\\", USER_DATA_DIR_NAME, "\\AGENTS.md");
#[cfg(not(target_os = "windows"))]
pub const GLOBAL_AGENTS_FILE_DISPLAY: &str =
    const_format::concatcp!("~/.config/", USER_DATA_DIR_NAME_LOWERCASE, "/AGENTS.md");
